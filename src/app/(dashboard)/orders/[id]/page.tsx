...
import { getOrder, updateOrderStatus, assignStaffToOrder, deleteOrder } from "@/actions/orders";
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {/* Totals */}
          <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-950/30 dark:to-violet-950/30 border border-indigo-100 dark:border-indigo-900/30 space-y-2">
            <div className="flex justify-between text-sm">
              <span>Subtotal</span><span className="font-semibold">{formatCurrency(order.subtotal)}</span>
            </div>
            {order.discount > 0 && (
              <div className="flex justify-between text-sm">
                <span>Discount</span><span className="text-rose-600 font-semibold">- {formatCurrency(order.discount)}</span>
              </div>
            )}
            <div className="flex justify-between text-base font-bold border-t border-indigo-200 dark:border-indigo-800 pt-2">
              <span>Total Amount</span><span className="text-indigo-600 dark:text-indigo-400">{formatCurrency(order.totalAmount)}</span>
            </div>
            {order.advancePayment > 0 && (
              <>
                <div className="flex justify-between text-sm">
                  <span>Advance Paid</span><span className="text-emerald-600 font-semibold">- {formatCurrency(order.advancePayment)}</span>
                </div>
                <div className="flex justify-between text-sm font-bold">
                  <span>Balance Due</span><span className="text-amber-600">{formatCurrency(order.balanceAmount)}</span>
                </div>
              </>
            )}
          </div>
