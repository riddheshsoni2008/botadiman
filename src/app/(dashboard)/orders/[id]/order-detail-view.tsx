...
export function OrderDetailView({
  ...
  const [selectedStaff, setSelectedStaff] = useState<string[]>(
    initialOrder?.assignedStaff?.map((a: any) => a.staff) || []
  );
  const [studioSettings] = useState<any>(initialStudioSettings);
  ...
  function toggleStaff(staffId: string) {
    setSelectedStaff((prev) =>
      prev.includes(staffId) ? prev.filter((sId) => sId !== staffId) : [...prev, staffId]
    );
  }

  async function handleAssignStaff() {
    const assignments = selectedStaff.map((staffId) => {
      const s = allStaff.find((staffMember) => staffMember._id === staffId);
      return {
        staffId,
        name: s?.name || "",
        role: s?.role || "",
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge
                ...
                className="gap-1 shadow-xs font-medium"
              >
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" size="sm" onClick={openStaffDialog} className="gap-1.5 rounded-xl">
          ...
          {(statusActions[order.status as keyof typeof statusActions] || []).map((action) => {
            ...
            return (
              <Button
                ...
                className={`gap-1.5 rounded-xl ${isStartWork && !isFullyStaffed ? "border border-amber-500/50 bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20" : ""}`}
          ...
          <Button variant="ghost" size="sm" onClick={handleDelete} className="text-rose-500 hover:text-rose-700 gap-1.5 rounded-xl">
      ...
      <Card className="rounded-2xl">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={openStaffDialog} className="gap-1 rounded-xl">
          ...
          {!isFullyStaffed && (
            <div className="mb-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-amber-700 dark:text-amber-400">
              ...
              <Button
                ...
                className="h-7 text-xs border-amber-500/30 text-amber-700 dark:text-amber-300 hover:bg-amber-500/20 shrink-0 rounded-lg"
              >
                Assign Now
              </Button>
            </div>
        ...
        <form onSubmit={handleUpdatePayment} className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              autoFocus
            />
          </div>
